  ...
  return (
    <>
      <div className="grid grid-cols-12">
        ...
        <div
          onClick={handleClick}
          className="col-span-12 border rounded mx-4 flex justify-center "
        >
          <div className="p-5 text-center cursor-pointer">
            ...
            <DynamicIcon
              size="100"
              name={subIcon}
              className="h-10 mt-4 mx-auto"
            ></DynamicIcon>
            <div
              className={`mt-3 ${pageType !== "baseline" ? "text-gray-400" : ""}`}
            >
              {uploadText}
            </div>
            <div className="mt-3 mr-2 text-gray-400">{noDataText}</div>
          </div>
        </div>
        {pageType === "scan" && (
